...
while [[ $STATUS -ne 0 ]] && [[ $i -lt 60 ]]; do
	...
	/opt/mssql-tools18/bin/sqlcmd -t 1 -S 127.0.0.1 -U sa -P $MSSQL_SA_PASSWORD -C -Q "select 1" >> /dev/null
	STATUS=$?
	sleep 1	
done
...
/opt/mssql-tools18/bin/sqlcmd -S 127.0.0.1 -U sa -P $MSSQL_SA_PASSWORD -C -d master -i /var/opt/mssql/setup/setup.instance.sql
...
then
	...
	/opt/mssql-tools18/bin/sqlcmd -S 127.0.0.1 -U sa -P $MSSQL_SA_PASSWORD -C -d master -i /var/opt/mssql/setup/setup.restore.sql
	...
	1)	echo "*********** Restoring big databases: WideWorldImportersDW, AdventureworksDW, StackOverflow..." | tee -a ./config.log
		/opt/mssql-tools18/bin/sqlcmd -S 127.0.0.1 -U sa -P $MSSQL_SA_PASSWORD -C -d master -i /var/opt/mssql/setup/setup.other_databases.restore.sql	
		/opt/mssql-tools18/bin/sqlcmd -S 127.0.0.1 -U sa -P $MSSQL_SA_PASSWORD -C -d master -i /var/opt/mssql/setup/setup.bigdatabases.restore.sql	
	;;
	*)  echo "INCLUDE_ALL_DATABASES is not set; defaulting to not include all databases." | tee -a ./config.log
	...
	1)	echo "*********** Attaching previously restored databases..." | tee -a ./config.log
		/opt/mssql-tools18/bin/sqlcmd -S 127.0.0.1 -U sa -P $MSSQL_SA_PASSWORD -C -d master -i /var/opt/mssql/setup/setup.attach.sql
		
		case $INCLUDE_ALL_DATABASES in	
		1)	echo "*********** Attaching previously restored big databases..." | tee -a ./config.log
			/opt/mssql-tools18/bin/sqlcmd -S 127.0.0.1 -U sa -P $MSSQL_SA_PASSWORD -C -d master -i /var/opt/mssql/setup/setup.other_databases.attach.sql
			/opt/mssql-tools18/bin/sqlcmd -S 127.0.0.1 -U sa -P $MSSQL_SA_PASSWORD -C -d master -i /var/opt/mssql/setup/setup.bigdatabases.attach.sql
		;;
		*)  echo "INCLUDE_ALL_DATABASES is not set; defaulting to not include all databases." | tee -a ./config.log
		;;
		esac	
